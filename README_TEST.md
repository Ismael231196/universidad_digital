## README_TEST - Validación de APIs (Pruebas Automáticas + Manuales)

### 1. Introducción

Este documento explica cómo **probar los endpoints del backend** usando:
- **Pruebas automáticas:** tests unitarios e integración con pytest
- **Pruebas manuales:** Swagger UI, Postman, cURL
...
```
tests/
├── unit/                    # Tests unitarios (lógica sin I/O)
│   ├── test_auth_services.py
│   ├── test_users_services.py       ✨ NUEVO
│   └── test_roles_services.py       ✨ NUEVO
├── integration/             # Tests de integración (APIs + BD)
│   ├── test_auth_routes.py
│   ├── test_users_routes.py         ✨ NUEVO
│   └── test_roles_routes.py         ✨ NUEVO
├── e2e/                     # Tests end-to-end
│   └── test_flujo_completo_usuario.py
...
# Instalar dependencias con pytest-cov
...
| Comando | Descripción |
|---------|-------------|
| `pytest -m unit` | Solo unitarios (rápido, ~5s) |
| `pytest -m integration` | Solo integración (~20s) |
| `pytest -m "unit or integration"` | Unit + Integration (~25s) |
| `pytest` | Todos (incluyendo E2E, ~60s) |
| `pytest -v` | Verbose (muestra cada test) |
| `pytest --cov=app` | Con reporte de cobertura |
| `pytest tests/unit/test_users_services.py` | Solo usuarios |
| `pytest tests/unit/test_roles_services.py` | Solo roles |

### Tests Nuevos (36 tests totales)

#### ✨ Module de Usuarios (21 tests)

**Unitarios (`test_users_services.py`):** 12 tests
- Crear usuario estudiante válido
- Email duplicado rechazado
- Admin requiere password ≥12 caracteres
- Asignar rol default
- Listar usuarios
- Obtener usuario por ID
- Actualizar usuario (nombre, password, roles, is_active)
- Desactivar usuario
- Asignar/remover roles

**Integración (`test_users_routes.py`):** 9 tests
- Crear usuario como admin (201)
- Crear sin admin (403)
- Listar usuarios
- Obtener por ID
- Actualizar usuario
...
#### ✨ Módulo de Roles (15 tests)

**Unitarios (`test_roles_services.py`):** 7 tests
- Crear rol válido
- Nombre duplicado rechazado
- Listar roles
- Obtener rol por ID
- Actualizar rol (nombre, descripción)
- Eliminar rol

**Integración (`test_roles_routes.py`):** 8 tests
- Crear rol como admin
- Crear sin admin (403)
- Listar, get, update, delete

#### ✨ Tests de Seguridad (32 tests nuevos)

**Unitarios (`test_security.py`):** 13 tests
- Autenticación: credenciales inválidas, usuario inactivo, inexistente
- Autorización: estudiante/docente no pueden crear usuarios
- Validación: email, password, SQL injection, XSS
- Ownership: usuario no puede modificar otro

**Integración (`test_security_routes.py`):** 19 tests  
- Login: exitoso, inválido, logout, protección de endpoints
- Autorización por rol: admin vs estudiante vs docente
- Sanitización: email inválido, password corta, campos requeridos
- Errores: no revelan información sensible

---

## RESUMEN TOTAL DE TESTS (80 tests)
...
# Unit + Integration (para CI)
pytest -m "unit or integration" -v --cov=app --cov-fail-under=85

# Con reporte detallado
pytest -m security --cov=app/auth --cov=app/core --cov-report=html
```

**Documentación detallada:** [docs/TESTING-SECURITY.md](docs/TESTING-SECURITY.md)

### Cobertura Local Expected
